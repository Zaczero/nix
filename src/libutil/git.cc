...
std::optional<LsRemoteRefLine> parseLsRemoteLine(std::string_view line)
{
    ...
    return LsRemoteRefLine{
        .kind = !match[1].matched ? LsRemoteRefLine::Kind::Object : LsRemoteRefLine::Kind::Symbolic,
        .target = std::string(match[2].first, match[2].second),
        .reference =
            !match[3].matched ? std::nullopt : std::optional<std::string>{std::string(match[3].first, match[3].second)},
    };
}
